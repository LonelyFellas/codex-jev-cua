...
0.7.2 起，pi 在 Agent 任务结束或取消时释放本插件的 Sky 连接并作废旧状态；下一次明确请求桌面操作时按需新建连接，仍走官方授权。不要自动重放上次操作、旋转会话身份或把断开当作撤销已发生动作。官方 `session_stopped` 是停止错误，不是可用窗口状态；若后续用户请求仍收到该错误，应报告并排查，不能承诺重连必然清除 Sky 的服务端停止状态。
