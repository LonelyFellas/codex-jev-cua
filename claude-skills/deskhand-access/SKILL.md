...
3. 要求 `envFile`、`appAccessFile`、`accessManagement.appsFile`、`accessManagement.cliPath` 均为绝对路径，`accessManagement.version=1`。缺字段说明旧服务不支持，提示升级到包含此功能的已发布版本并在 `/mcp` 重连，**不猜版本、路径，不自行升级或改用 pi Skill**。
