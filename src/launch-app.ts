...
export const launchAppSpec: NativeSpec = {
  ...
  description: "Launch or activate any installed app via macOS LaunchServices when the user requested opening it, before reading its window. Use an exact registered app name (identityType=name) or Bundle ID (identityType=bundleId), within the current task and configured app-access scope. No stateId is needed; consumes one action and invalidates previous state. Does not read UI, grant Sky/system permission, or prove window readiness. Next use cua_get_app_state to verify. Never use as a fallback after denied approval or an unknown outcome; no automatic retry. No URLs, files or extra launch arguments.",
