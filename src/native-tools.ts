...
const stateId = Type.String({ minLength: 1, maxLength: 80, description: "Single-use stateId from the latest cua_get_app_state or validated action-returned state for this app. If the action returns no new stateId, observe again." });
...
export const nativeSpecs: NativeSpec[] = [
  ...
  { name: "cua_get_app_state", method: "get_app_state", readOnly: true, parameters: schema({ app, disableDiff: Type.Optional(Type.Boolean()) }), description: "Read native Sky app/window/menu state and screenshot when no current stateId is available. Complete validated action results may return a new stateId, avoiding a duplicate read. Includes diagnostics; task budget is 180s/30 action attempts per agent turn. Text capped at 50 KB/2000 lines. No Jev call; official approval applies." },
