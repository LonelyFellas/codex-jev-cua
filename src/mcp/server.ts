...
const extra = [
  { name: "cua_status", description: "Native-only status and task diagnostics. No desktop access, no Jev, no permissions granted.", parameters: object({}) },
...
export function createNativeMcpServer(deps?: Dependencies) {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request, context) => {
    ...
    try {
      if (tool.name === "cua_status") return { content: [{ type: "text", text: JSON.stringify(session.status()) }] };
