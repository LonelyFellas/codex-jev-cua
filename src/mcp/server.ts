...
const extra = [
  { name: "cua_status", description: "Native-only status, current version and task diagnostics. Native task limits/remaining caps are null (unlimited), not exhausted; elapsed time/actions remain tracked. Offline by default. checkUpdates=true explicitly checks the public npm registry (5s timeout) and reports latest version and Plugin upgrade guidance; never installs or changes config/permissions. No desktop access or Jev.", parameters: object({ checkUpdates: Type.Optional(Type.Boolean()) }) },
  { name: "cua_task_begin", description: "Begin one user-authorized native task for one exact app, without a task-level time or action-count cap. Individual call timeouts and cancellation still apply. Always asks the human via elicitation. Never start a new task to bypass a refusal.", parameters: object({ app: Type.String({ minLength: 1, maxLength: 200 }), goal: Type.String({ minLength: 1, maxLength: 2000 }) }) },
