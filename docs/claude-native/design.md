...
- 配置路径为 DESKHAND_CONFIG_FILE 或 ~/.config/deskhand/cua.env；读取现有配置/授权文件格式但强制 native，不加载 Jev 决策器。不自动创建配置或修改既有授权文件；新源码默认 all（0.4.0 原为仅 Calculator），已有显式 allowlist 保持优先，用户可用专用权限 Skill 明确切换。
